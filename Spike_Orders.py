...
def get_coordinates(row):
    ...
    for i, element in enumerate(row):
        if element == 1:
            ones_coordinates.append(i)
    return ones_coordinates
def spike_train_order_matrix(spike_trains, all_trains):
    num_trains = len(spike_trains)
    ss = ss_plot(spike_trains, all_trains)
    print(ss)
    sto = np.zeros((ss.shape[0], ss.shape[1]))
    for i in range(num_trains):
        one_coordinates = get_coordinates(ss[i])
        j = 0
        
        print(one_coordinates)
       
        while j < (len(one_coordinates)):
            y = one_coordinates[j]
            z = one_coordinates[j+1]
            x = all_trains[y] 
            w = all_trains[z] 
            ...
            if x < w :
                ...
                sto[i, y] = -1
                sto[i, z] = -1
            print(y, z,x, w,i, j, len(one_coordinates), sto)
            j = j+2
    return sto
                  
sto = spike_train_order_matrix(spike_trains, all_trains)
print(so_matrix(ss))
print(sto)